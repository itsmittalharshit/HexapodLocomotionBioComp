"""
v3_proprioception.py  –  CPG Hexapod + Foraging + Proprioception (Point 2 + 4)
===============================================================================
Variant 3: Adds proprioceptive sensory feedback to the Foraging base.

NEW vs v1_foraging:
  - CPGNetwork.step(dt, feedback=None)
      feedback : 6-element float array; per-leg collision signal [0,1]
      When feedback[i] > threshold, leg i's phase is immediately snapped
      to the Swing onset (φ = 0 mod 2π), terminating Stance and
      beginning recovery — the biological "stumble-and-recover" reflex.
  - CPGHexapod.on_collision() : sends feedback signal for all legs
      close to the collision side (left/right hemisphere)
  - CPGHexapod.control()      : passes accumulated feedback to cpg.step()
  - Config.FEEDBACK_*         : threshold and decay constants

Biological rationale
  Real insects use campaniform sensilla (force sensors) on each leg to
  detect unexpected loads and immediately trigger swing phases. This
  makes the gait reactive rather than purely feed-forward.
"""
...
IS_DEMO    = True
DEMO_NAME  = "CPG Hexapod – Proprioception + Foraging"
...
class Config:
    ...
    GENOME_SAVE_PATH  = "best_hexapod_proprioception.json"
    ...
    # ── Proprioception / feedback parameters ─────────────────────────────
    FEEDBACK_THRESHOLD  = 0.5   # signal level that triggers phase reset
    FEEDBACK_DECAY      = 0.85  # per-step decay of feedback signal (< 1)
    FEEDBACK_MAGNITUDE  = 1.0   # initial signal strength on collision
...
# ══════════════════════════════════════════════════════════════════════════════
# CPG NETWORK  (with proprioceptive feedback)
# ══════════════════════════════════════════════════════════════════════════════

class CPGNetwork:
    """
    Kuramoto-coupled oscillator network with sensory feedback reset.

    When feedback[i] exceeds FEEDBACK_THRESHOLD, the oscillator's phase
    is snapped to the beginning of the Swing phase (φ ≡ ε, sin > 0),
    immediately lifting the leg off the obstacle — the stumble reflex.
    """
    ...
    def step(self, dt, feedback=None):
        """
        Parameters
        ----------
        dt       : float  – time delta
        feedback : array-like of length 6 (per-leg load signal) or None
        """
        # Apply reflexive phase resets before integration
        if feedback is not None:
            thr = Config.FEEDBACK_THRESHOLD
            for i in range(6):
                if feedback[i] > thr:
                    # Snap to swing onset: sin(φ) just above 0
                    self.phi[i] = self.desired[i] + 0.05  # tiny positive offset
...
class CPGHexapod(Agent, Evolver):
    ...
    def __init__(self):
        ...
        # Per-leg proprioceptive feedback signal (decays each step)
        self._feedback = np.zeros(6, dtype=np.float64)
        self._reflex_count = 0    # how many reflexes fired this assessment
    ...
    def get_fitness(self):
        ...
        # Bonus for reflexes fired: rewards reactive recovery
        reflex_bonus = self._reflex_count * 0.3
        return max(0.0, base + explore - penalty + reflex_bonus)
    ...
    def on_collision(self, other):
        """
        Proprioception: collision raises load-signal for the legs on the
        relevant body-side. The CPG's stumble-reflex handles the rest.
        """
        if isinstance(other, Obstacle):
            self._collision_penalty += 1.0
            # Inject feedback into all legs (robot hit something)
    ...
    def control(self):
        ...
        # Step CPG with current proprioceptive feedback
        self.cpg.step(dt, feedback=self._feedback)

        # Decay feedback signal
    ...
    def draw(self):
        ...
        # Draw feedback indicator (red halo when reflex fires)
        self._draw_feedback_halo()
        glDisable(GL_LINE_SMOOTH)

    def _draw_feedback_halo(self):
        """Flash a red halo when proprioceptive reflex is active."""
    ...
    def _draw_leg(self, i):
        ...
        # Highlight legs in reflex state
        fb_intensity = self._feedback[i]
        if fb_intensity > Config.FEEDBACK_THRESHOLD:
        ...
        cx = ax+side*10.; cy = ay
        kx = ax+side*(10.+14.*0.5); ky = ay+14.*0.3+lift*0.4
        fx = ax+side*(10.+14.*0.6+sweep); fy = ay+14.*0.5+lift
        glLineWidth(2.)
        glBegin(GL_LINE_STRIP)
        glVertex2d(ax,ay); glVertex2d(cx,cy); glVertex2d(kx,ky); glVertex2d(fx,fy)
        glEnd()
        glColor4fv([0.,1.,0.5,1.] if not swing else [1.,0.6,0.1,.8])
...
class CPGHexapodSimulation(Simulation):
    def __init__(self):
        super().__init__("CPGHexapod_Proprioception")
    ...
    def begin_simulation(self):
        print("=" * 60)
        print("  CPG Hexapod – Proprioception + Foraging")
        print(f"  Feedback decay    : {Config.FEEDBACK_DECAY}")
        print(f"  Reflex threshold  : {Config.FEEDBACK_THRESHOLD}")
